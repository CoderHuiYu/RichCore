...
extension String {
    ...
    public func withPeogooFont(of size: PGFontSize, weight: PGFontWeight = .regular, color: UIColor? = nil, alignment: NSTextAlignment? = nil, lineSpace: CGFloat = 0.0) -> NSMutableAttributedString {
        let font = UIFont.peogooFont(size: size, weight: weight)
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = lineSpace
        paragraphStyle.minimumLineHeight = size.lineHeight
        paragraphStyle.maximumLineHeight = size.lineHeight
        if size == .size12 { paragraphStyle.maximumLineHeight += 3 } // Quick fix for emoji
...
extension Dictionary where Key == NSAttributedString, Value == Any {
    public static func zillyFont(of size: PGFontSize, weight: PGFontWeight = .regular, color: UIColor? = nil, alignment: NSTextAlignment? = nil) -> [NSAttributedString.Key:Any] {
        let font = UIFont.peogooFont(size: size, weight: weight)
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.minimumLineHeight = size.lineHeight
        paragraphStyle.maximumLineHeight = size.lineHeight
