...
@objc(PGButton) open class PGButton : UIButton {
    ...
    public enum HighlightStyle {
        case alphaChannel
        case custom(handler: (_ isHighlighted: Bool, _ button: PGButton) -> ActionClosure)
        case none
    }
    ...
    /// Adjust the image and the button's title label vertically centered
    public func adjustImageAndTitleVertically(imageGapToTitle gap: CGFloat = 0.0) {
        ...
        if let titleLabel = titleLabel, let imageView = imageView, let buttonTitle = titleLabel.text, let image = imageView.image {
            ...
            let titleSize = titleString.size(withAttributes:[ NSAttributedString.Key.font : titleLabel.font! ])
            let buttonImageSize = image.size
            let topImageOffset = -(buttonSize.height - (titleSize.height + buttonImageSize.height))
            let leftImageOffset = (buttonSize.width - buttonImageSize.width) / 2
            imageEdgeInsets = UIEdgeInsets(top: topImageOffset, left: leftImageOffset, bottom: 0, right: 0)
            
            let topTitleOffset = topImageOffset + 2*buttonImageSize.height + 2*gap
            let leftTitleOffset = (buttonSize.width - titleSize.width) / 2 - image.size.width - leftImageOffset/2
